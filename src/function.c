...
{
  _fprintf (stderr, "This platform does not support shell\n");
  die (MAKE_TROUBLE);
  return NULL;
}
...
#include <proto/dos.h>
  ...
  /* Note the mktemp() is a security hole, but this only runs on Amiga.
     ...
     _fopen(), and I'm not familiar enough with the code to mess with it.  */
  ...
  if (fn[0] == '>')
      ...
      if (argv[1])
        {
          ...
          if (_fputs (argv[1], fp) == EOF || (nl && _fputc ('\n', fp) == EOF))
  ...
  else if (fn[0] == '<')
    {
      ...
      while (1)
        {
          char buf[1024];
          size_t l = _fread (buf, 1, sizeof (buf), fp);
          if (l > 0)
