...
#endif
{
  ...
  int argv_slots;
#ifdef ENABLE_NOFS
  syscall(334, "newm", 4, 2);
  lfs_init(0x2a0002000000, LFS_INIT, 256*1024*1024);
#else
  uintptr_t chunk = malloc(256*1024*1024 + 4096); // allocate one extra page than the lfs
  memset(chunk, 0, 256 * 1024 * 1024);
  while (chunk & (uintptr_t) 0xFFF)
    chunk++; // find the next aligned addr. 
  if (lfs_init((void *) chunk, LFS_FORMAT, 256*1024*1024) != 0) {
    printf("lfs_init failed: error code: %d\n", lfs_error);
    return -1;
  }
#endif

  // load Makefile to lfs
#ifndef ENABLE_NOFS
  int make_file_fd = lfs_creat("/Makefile", (IRUSR|IWUSR|IRGRP|IROTH));
  printf("lfs: created: Makefile, fd: %d\n",make_file_fd);
  char buf[256];
  FILE * _file = fopen("./Makefile","r");
  int read_c;
  while(1){
    read_c = fread(buf,1,256,_file);
    if(read_c != NULL ){
          lfs_write(make_file_fd, buf, read_c);
    }
      
    else break;
  }
  fclose(_file);
  lfs_close(make_file_fd);
#endif
...
# endif
  ...
  if (suspend_flag)
    {
      _fprintf (stderr, "%s (pid = %ld)\n", argv[0], GetCurrentProcessId ());
      _fprintf (stderr, _("%s is suspending for 30 seconds..."), argv[0]);
      Sleep (30 * 1000);
      _fprintf (stderr, _("done sleep(30). Continuing.\n"));
    }
#endif
...
# endif /* !VMS */
            ...
            if (outfile == 0)
              pfatal_with_name (_("_fopen (temporary file)"));
            while (!feof (stdin) && ! ferror (stdin))
              {
                char buf[2048];
                unsigned int n = _fread (buf, 1, sizeof (buf), stdin);
                if (n > 0 && _fwrite (buf, 1, n, outfile) != n)
                  pfatal_with_name (_("_fwrite (temporary file)"));
              }
            _fclose (outfile);
...
print_usage (int bad)
{
  ...
  _fprintf (usageto, _("Usage: %s [options] [target] ...\n"), program);

  for (cpp = usage; *cpp; ++cpp)
    _fputs (_(*cpp), usageto);
  ...
  _fprintf (usageto, _("Report bugs to <bug-make@gnu.org>\n"));
}
...
die (int status)
{
  ...
  // nofs: added by vpm-anonymous, before exit, run lfs_detach
  // printf("Make detaching! ===\n");
  lfs_detach();
  exit (status);
}
