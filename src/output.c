...
/* Close standard output, exiting with status 'exit_failure' on failure.
   ...
   the _fclose(stdout) could still fail (due e.g., to a disk full error)
   when it tries to write out that buffered data.  Thus, you would be
   left with an incomplete output file and the offending program would
   exit successfully.  Even calling _fflush is not always sufficient,
...
close_stdout (void)
{
  ...
  int _fclose_fail = _fclose (stdout);

  if (prev_fail || _fclose_fail)
    {
      if (_fclose_fail)
