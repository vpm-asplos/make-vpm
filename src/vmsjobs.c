  ...
  if (vms_always_use_cmd_file || token.use_cmd_file)
    {
      ...
      _fprintf (outfile, "$ gnv$$make_verify = \"''f$verify(0)'\"\n");
      _fprintf (outfile, "$ gnv$$make_pid = f$getjpi(\"\",\"pid\")\n");
      _fprintf (outfile, "$ on error then $ goto gnv$$make_error\n");
      ...
      _fprintf (outfile, "$ gnv$$make_verify = f$verify(gnv$$make_verify)\n");
      ...
      */
      ...
      _fprintf (outfile, "%s\n", cmd_dsc->dsc$a_pointer);
      _fprintf (outfile, "$ gnv$$make_status_2 = $status\n");
      _fprintf (outfile, "$ goto gnv$$make_exit\n");

      /* Exit and clean up */
      _fprintf (outfile, "$ gnv$$make_error: ! 'f$verify(0)\n");
      _fprintf (outfile, "$ gnv$$make_status_2 = $status\n");

      if (append_token != -1)
        {
          _fprintf (outfile, "$ deassign sys$output\n");
          _fprintf (outfile, "$ close 'gnv$$make_al'\n");

          DB (DB_JOBS,
              (_("Append %.*s and cleanup\n"), comnamelen-3, child->comname));
        }
      _fprintf (outfile, "$ gnv$$make_exit: ! 'f$verify(0)\n");
      _fprintf (outfile,
