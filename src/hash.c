           ...
           hash_func_t hash_1, hash_func_t hash_2, hash_cmp_func_t hash_cmp)
{
  ...
  if (ht->ht_vec == 0)
    {
      _fprintf (stderr, _("can't allocate %lu bytes for hash table: memory exhausted"),
...
{
  _fprintf (out_FILE, _("Load=%lu/%lu=%.0f%%, "), ht->ht_fill, ht->ht_size,
	   100.0 * (double) ht->ht_fill / (double) ht->ht_size);
  _fprintf (out_FILE, _("Rehash=%u, "), ht->ht_rehashes);
  _fprintf (out_FILE, _("Collisions=%lu/%lu=%.0f%%"), ht->ht_collisions, ht->ht_lookups,
	   (ht->ht_lookups
